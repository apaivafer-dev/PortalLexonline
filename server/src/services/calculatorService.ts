...
/**
 * Realiza o cálculo completo de uma rescisão trabalhista com base nos parâmetros da CLT.
 * 
 * @param input Dados de entrada incluindo datas, salário, motivo da rescisão e adicionais.
 * @returns Um objeto contendo a lista detalhada de verbas (items), totais de proventos, descontos, valor líquido e a data projetada de término.
 * 
 * @rule Compliance: Rule 6 (Clean Architecture) - Business logic centralizada no service.
 * @rule Compliance: Rule 14 (Documentation) - Docstrings descritivas.
 */
export const calculateRescisao = (input: CalculatorInput): CalculationResult => {
    ...
    // Night Shift (Estimated 20% if checked)
    const nightPay = input.additionalNight ? salary * 0.20 : 0;
    const dsrNightPay = input.additionalNight ? nightPay / 6 : 0;

    // Overtime
    const overtimePay = input.additionalHours;
    const dsrOvertimePay = overtimePay > 0 ? overtimePay / 6 : 0;
    ...
    if (input.terminationType !== TerminationType.JUSTA_CAUSA) {
        ...
        let diffDays = Math.ceil(Math.abs(projectedEnd.getTime() - lastAnniversary.getTime()) / (1000 * 60 * 60 * 24));
        let monthsVacation = Math.floor(diffDays / 30);
        if (diffDays % 30 >= 15) monthsVacation += 1;
    ...
    if (input.terminationType !== TerminationType.PEDIDO_DEMISSAO && input.terminationType !== TerminationType.JUSTA_CAUSA) {
        const earningsForGeneralFGTS = items.filter(i =>
            ...
            i.group !== 'FGTS' &&
            i.group !== 'Férias' &&
            !i.description.includes('Aviso Trabalhado')
