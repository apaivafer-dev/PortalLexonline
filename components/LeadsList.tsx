...
export const LeadsList = ({ leads, pipelines, onUpdateLeads, onUpdatePipelines, initialOpenLeadId, onClearTarget }: LeadsListProps) => {
  ...
  // Initialize CRM Data Mock
  useEffect(() => {
      // Generate realistic mock data if empty
      if(contacts.length === 0 && crmCompanies.length === 0) {
          const mockCompanies: CRMCompany[] = [
              { id: 'comp-1', name: 'Tech Solutions Ltda', tradeName: 'TechSol', emails: ['contato@techsol.com'], phones: [{number:'1133334444', type:'Fixo'}], types: ['Cliente'], contactIds: ['cont-1', 'cont-2'], createdAt: new Date().toISOString() },
              { id: 'comp-2', name: 'Comércio de Papéis Silva', tradeName: 'Papelaria Silva', emails: ['vendas@papeis.com'], phones: [{number:'1133335555', type:'Fixo'}], types: ['Fornecedor'], contactIds: ['cont-3'], createdAt: new Date().toISOString() },
              { id: 'comp-3', name: 'Inovações Digitais S.A.', tradeName: 'InovaDigital', emails: ['hello@inova.com'], phones: [{number:'1133336666', type:'Fixo'}], types: ['Parceiro', 'Cliente'], contactIds: ['cont-4'], createdAt: new Date().toISOString() }
          ];

          const firstNames = ['Roberto', 'Ana', 'Carlos', 'Fernanda', 'João', 'Maria', 'Pedro', 'Lucas', 'Juliana', 'Marcos'];
          const lastNames = ['Almeida', 'Costa', 'Pereira', 'Santos', 'Oliveira', 'Souza', 'Lima', 'Ferreira', 'Rodrigues', 'Gomes'];
          
          const mockContacts: CRMContact[] = Array.from({ length: 10 }).map((_, i) => ({
              id: `cont-${i+1}`,
              firstName: firstNames[i],
              lastName: lastNames[i],
              cpf: `123.456.789-0${i}`,
              emails: [`${firstNames[i].toLowerCase()}@email.com`],
              phones: [{ number: `1199999${1000 + i}`, type: 'Móvel' }],
              type: i < 5 ? 'Cliente' : 'Lead',
              companyId: i === 0 || i === 1 ? 'comp-1' : (i === 2 ? 'comp-2' : (i === 3 ? 'comp-3' : undefined)),
              createdAt: new Date().toISOString()
          }));

          setCrmCompanies(mockCompanies);
          setContacts(mockContacts);
          
          // Initial Custom Fields Mock
          setCustomFields([
              { id: 'cf-1', key: 'nascimento', label: 'Data de Aniversário', type: 'date', entityType: 'contact', group: 'Informações Pessoais', required: false, multiple: false },
              { id: 'cf-2', key: 'setor', label: 'Setor', type: 'select', entityType: 'company', group: 'Dados Gerais', required: false, multiple: false, options: [{id:'1', label:'Tecnologia', value:'tech'}, {id:'2', label:'Varejo', value:'retail'}] },
              { id: 'cf-3', key: 'whatsapp_btn', label: 'WhatsApp', type: 'button', entityType: 'lead', group: 'Comunicação', required: false, multiple: false, buttonConfig: { label: 'Chamar no Whats', color: '#25D366', icon: 'whatsapp', urlTemplate: 'https://wa.me/{phone}?text=Olá {name}, tudo bem?' } }
          ]);

          // Initial Tags Mock
          setTags([
              { id: 'tag-1', name: 'Quente', color: '#EF4444', entityType: 'lead' },
              { id: 'tag-2', name: 'Morno', color: '#F59E0B', entityType: 'lead' },
              { id: 'tag-3', name: 'Cliente VIP', color: '#8B5CF6', entityType: 'contact' },
              { id: 'tag-4', name: 'Inadimplente', color: '#64748B', entityType: 'company' }
          ]);
      }
  }, []);
  ...
  const handleSaveContact = (contact: CRMContact) => {
      if (editingContact) {
          ...
          setContacts(prev => [...prev, { ...contact, id: generateId(), createdAt: new Date().toISOString() }]);
      }
  ...
  const handleDeleteContact = (id: string) => {
      if(confirm('Deseja excluir este contato?')) setContacts(prev => prev.filter(c => c.id !== id));
  };

  const handleSaveCompany = (company: CRMCompany) => {
      if (editingCompany) {
          setCrmCompanies(prev => prev.map(c => c.id === company.id ? company : c));
      } else {
          setCrmCompanies(prev => [...prev, { ...company, id: generateId(), createdAt: new Date().toISOString() }]);
      }
  ...
  const handleDeleteCompany = (id: string) => {
      if(confirm('Deseja excluir esta empresa?')) setCrmCompanies(prev => prev.filter(c => c.id !== id));
  };
  ...
  const UnifiedConfigModal = () => {
      ...
      const fieldGroups = Array.from(new Set(filteredEntityFields.map(f => f.group)));
  ...
  const LeadFormModal = () => {
    ...
    return (
        <div className="w-full space-y-6 h-[calc(100vh-140px)] flex flex-col">
            {/* ... Rest of Main Render (Top Bar, Content Area) remains unchanged ... */}
            
            {/* TOP BAR: TABS & ACTIONS */}
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-[#1A1D23] p-4 rounded-[24px] shadow-sm border border-slate-100 dark:border-white/5 flex-shrink-0">
                
                {/* Tabs */}
                <div className="flex p-1 bg-slate-100 dark:bg-white/5 rounded-xl overflow-x-auto max-w-full">
                    {[
                ...
                <div className="flex items-center gap-3 w-full xl:w-auto overflow-x-auto">
                        ...
                        <>
                            ...
                            <div className="relative group">
                                <select 
                                    ...
                                    className="appearance-none pl-4 pr-10 py-2.5 bg-slate-50 dark:bg-white/5 border border-transparent hover:border-slate-200 dark:hover:border-white/10 rounded-xl text-sm font-bold text-slate-700 dark:text-white outline-none cursor-pointer min-w-[180px]"
                                >
                            ...
                            <div className="flex bg-slate-50 dark:bg-white/5 p-1 rounded-xl border border-slate-100 dark:border-white/5">
                                <button onClick={() => setViewMode('board')} className={`p-2 rounded-lg ${viewMode === 'board' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-white' : 'text-slate-400'}`}><LayoutGrid size={18} /></button>
                                <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-white' : 'text-slate-400'}`}><ListIcon size={18} /></button>
                            </div>
                        </>
                    )}
                    ...
                    <button 
                        ...
                        className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-white/5 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                        title="Configurações"
                    >
                        <Settings size={18} />
                    </button>
                    ...
                    <button 
                        ...
                        className="h-10 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg shadow-indigo-500/20 transition-all"
                    >
                        <Plus size={18} /> 
                        <span className="hidden sm:inline">
                            {activeTab === 'oportunidades' ? 'Novo Lead' : (activeTab === 'contatos' ? 'Novo Contato' : (activeTab === 'empresas' ? 'Nova Empresa' : 'Nova Automação'))}
                        </span>
                    </button>
