...
// Generate 12 Mock Banners
const generateMockBanners = (initialCompanyName: string): BannerConfig[] => {
    return Array.from({ length: 12 }).map((_, i) => {
        const date = new Date();
        date.setDate(date.getDate() - i * 3); // Spread dates back
        date.setHours(14, 30 + i, 0);
        
        return {
            id: `mock-${i}`,
            createdAt: date.toISOString(),
            ...DEFAULT_CONFIG,
            companyName: i === 0 ? initialCompanyName : `Empresa Exemplo ${i + 1}`,
            bannerColor: BANNER_COLORS[i % BANNER_COLORS.length],
            bannerTitle: i % 2 === 0 ? 'Avalie-nos no Google' : 'Deixe sua Opinião',
        };
    });
};

// Sub-component for Mini Preview (Gallery Item)
const MiniBannerPreview = ({ config, onClick, isLocked }: { config: BannerConfig, onClick: () => void, isLocked?: boolean }) => {
        ...
        >
            ...
            <div className="relative w-full pt-[141%] bg-slate-100 dark:bg-slate-900 rounded-xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700 transition-all duration-300 group-hover:shadow-lg group-hover:border-indigo-300 group-hover:scale-[1.02]">
                ...
                >
                    ...
                    <p className="text-[5px] italic opacity-90 mt-auto font-medium line-clamp-2">{config.instructions}</p>
                </div>
...
export const BannerCreator = ({ initialCompanyName = 'Minha Empresa', userProfile }: BannerCreatorProps) => {
    ...
    useEffect(() => {
        const mocks = generateMockBanners(initialCompanyName);
        setBanners(mocks);
        
        // If no banners, go straight to editor (only if creating is allowed)
        if (mocks.length === 0) {
            handleCreateNew();
        }
    }, [initialCompanyName]);
    ...
    const handleSaveAndDownload = async () => {
        ...
        try {
            ...
            setBanners(prev => {
                if (currentId) {
                    return prev.map(b => b.id === currentId ? { ...newConfig, id: currentId, createdAt: b.createdAt } : b);
                } else {
