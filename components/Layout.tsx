...
export const Layout = ({ children, activePage, setActivePage, userProfile, onLogout, notifications = [], onOpenLead }: { 
  ...
  if (userProfile?.isAdmin) {
      navItems.push({ id: 'admin', label: 'Administrador', icon: <ShieldAlert size={20} /> });
  }

  const handleNavClick = (id: string) => {
    // Check restrictions for Trial users
    const isRestricted = userProfile?.plan === 'Trial' && (id === 'calculator' || id === 'leads');
    
    if (isRestricted) {
        alert("Funcionalidade exclusiva para planos Pro ou Premium. Por favor, faça o upgrade para acessar.");
        return;
    }
      ...
      >
        ...
        <div className={`h-24 flex items-center transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-8'}`}>
          <div className="flex items-center gap-3 cursor-pointer overflow-hidden whitespace-nowrap" onClick={() => setActivePage('dashboard')}>
            ...
            <span className={`font-bold text-xl text-slate-900 dark:text-white tracking-tight transition-opacity duration-300 ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100'}`}>
              LexOnline
            </span>
          </div>
        </div>
